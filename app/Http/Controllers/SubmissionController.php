...
class SubmissionController extends Controller
{
    ...
    public function update(Request $request, Submission $submission): JsonResponse
    {
        ...
        DB::transaction(function () use ($submission, $data) {
            ...
            // Replace system accesses
            $submission->systemAccesses()->delete();
            foreach (($data['system_accesses'] ?? []) as $sa) {
                $submission->systemAccesses()->create($sa);
            }

            // Detach file uploads from old colleagues before deleting them
            FileUpload::where('uploadable_type', Colleague::class)
                ->whereIn('uploadable_id', $submission->colleagues()->pluck('id'))
                ->update(['uploadable_type' => null, 'uploadable_id' => null]);

            // Replace colleagues (cascade deletes manual_tasks and missing_features)
            $submission->colleagues()->delete();
            foreach (($data['colleagues'] ?? []) as $colData) {
                $colleague = $submission->colleagues()->create([
                    'sort_order' => $colData['sort_order'] ?? 0,
                    'name' => $colData['name'] ?? null,
            ...
            // Re-associate overall file uploads
            $submission->fileUploads()->update(['uploadable_type' => null, 'uploadable_id' => null]);
            if (!empty($data['overall_file_ids'])) {
                FileUpload::whereIn('id', $data['overall_file_ids'])
                    ->update([
                        'uploadable_type' => Submission::class,
